-- Unified Database Schema for InvestoBot
-- This schema combines all previous migrations into a single unified schema
-- Run this SQL in your Supabase SQL editor to set up the complete database
--
-- This schema includes:
-- - Strategy runs and strategies
-- - Backtest results and metrics
-- - Risk assessments and execution results
-- - Portfolio snapshots
-- - Trades, fills, and run metrics (from observability schema)
-- - Data management tables (data sources, metadata, quality reports)
...
-- Combines definitions from both schemas (includes status field)
...
-- Combines both schema definitions (supports both run_id and strategy_id references)
CREATE TABLE IF NOT EXISTS backtest_results (
    ...
    strategy_spec JSONB,  -- Full strategy specification (from observability schema)
    ...
    metrics JSONB NOT NULL DEFAULT '{}',  -- Additional metrics as JSONB (from observability)
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);
...
-- ============================================================================
-- OBSERVABILITY TABLES (from 001_observability_schema.sql)
-- ============================================================================
